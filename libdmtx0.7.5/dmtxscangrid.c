...
InitScanGrid(DmtxDecode *dec)
{
   ...
   scale = dmtxDecodeGetProp(dec, DmtxPropScale); //解码得到支持
   smallestFeature = dmtxDecodeGetProp(dec, DmtxPropScanGap) / scale;//最小的特性

   grid.xMin = dmtxDecodeGetProp(dec, DmtxPropXmin);//网格x min
   grid.xMax = dmtxDecodeGetProp(dec, DmtxPropXmax);//网格 x max
   ...
   maxExtent = (xExtent > yExtent) ? xExtent : yExtent;//如果x大于y,返回x,否则返回y
